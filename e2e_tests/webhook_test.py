...
class WebhookE2eTests(unittest.TestCase):
    ...
    # payload = json.dumps(client.events.get(613457).data, cls=UnitEncoder)
    payload = "{data: [{id: 613457, type: authorizationRequest.pending," \
              ...
              "status: Pending, partialApprovalAllowed: true, merchant: {name: Apple Inc., type: 1000 }," \
              " recurring: false }, relationships: { authorizationRequest: {" \
              " data: { id: 4474, type: purchaseAuthorizationRequest }}, account: { data: { " \
              "id: 49228, type: account } }, customer: { data: { id: 49430, type: customer } }," \
