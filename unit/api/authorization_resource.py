...
class AuthorizationRequestResource(BaseResource):
    def __init__(self, api_url, token):
        ...
        self.resource = "authorizations-requests"
    ...
    def approve(self, authorization_id: str) -> Union[UnitResponse[PurchaseAuthorizationRequestDTO], UnitError]:
        response = super().post(f"{self.resource}/{authorization_id}/approve")
        if super().is_20x(response.status_code):
            data = response.json().get("data")
            return UnitResponse[PurchaseAuthorizationRequestDTO](DtoDecoder.decode(data), None)
        else:
    ...
    def decline(self, authorization_id: str) -> Union[UnitResponse[PurchaseAuthorizationRequestDTO], UnitError]:
        response = super().post(f"{self.resource}/{authorization_id}/decline")
        if super().is_20x(response.status_code):
            data = response.json().get("data")
            return UnitResponse[PurchaseAuthorizationRequestDTO](DtoDecoder.decode(data), None)
        else:
